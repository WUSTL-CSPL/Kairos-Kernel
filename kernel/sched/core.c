...
struct shore_task_info {
    int shore_id;             // Process ID
...
void shore_cpu_task_unlock(struct shore_task_info *entry) {
	raw_spin_unlock(&entry->lock);
}

struct shore_task_info *shore_find_id_helper(int find_shore_id) {
	...
	list_for_each_entry_safe(entry, tmp, &shore_cpu_task_list, list) {
		if (entry->shore_id == find_shore_id) {
			return entry;
		}
	}
	return NULL;
...
void shore_add_cpu_task_to_dfa_list(int new_shore_id, int new_shore_priority) {
    struct shore_task_info *new_entry;

    new_entry = kmalloc(sizeof(*new_entry), GFP_KERNEL);
    if (!new_entry) {
        pr_err("Failed to allocate memory for shore_task_info\n");
        return;
    }
...
void shore_remove_cpu_task_from_dfa_list(int new_shore_id) {
	struct shore_task_info *entry, *tmp;

	
	// Use list_for_each_entry_safe to iterate over each entry of the list
	list_for_each_entry_safe(entry, tmp, &shore_cpu_task_list, list) {
		if (entry->shore_id == new_shore_id) {
			list_del(&entry->list);
			kfree(entry);
			break;
		}
...
void shore_remove_all_cpu_tasks_from_dfa_list(void) {
	struct shore_task_info *entry, *tmp;
...
int shore_set_cpu_task_priority(int shore_id, int shore_priority) {
	struct shore_task_info *entry;
...
EXPORT_SYMBOL(shore_set_cpu_task_priority);
...
pick_next_task(struct rq *rq, struct task_struct *prev, struct rq_flags *rf)
{
	...
	if (likely(prev->sched_class <= &fair_sched_class &&
		...
		if (tmp && p) {
			...
			else if (tmp->shore_priority == ABORTED) {
				// set_user_nice(p, tmp->original_priority);
	...
	for_each_class(class) {
		...
		if (p) {
			...
			if (tmp) {
				...
				// printk(KERN_INFO "[Shore Eval] KERNAL TIME: %llu\n\n", pick_task_time);
				if (tmp->shore_priority == PRIOTIZED) {
					tmp->original_priority = p->static_prio;
...
#ifdef CONFIG_DataflowAvailability_SCHED
	printk(KERN_INFO "DFA Cross-layer scheduler v5.11.");
#endif
